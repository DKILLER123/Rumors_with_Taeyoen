...
(async()=>{
 ...
 for(const width of [390,800]){
  ...
  await page.goto(pathToFileURL(path.join(root,'work_epub/OEBPS/text/ch308.xhtml')).href);
  ...
  await page.screenshot({path:path.join(cache,`ch308-title-${width}.png`)});
  for(const [name,selector,index] of [['phone','.phone-call',0],['dossier','.dossier-block',0],['quest','.dev-quest',3],['wardrobe','.wardrobe-block',1],['status','.system-block',1],['ending','.performance-block',-1],['whisper','.whisper-block',0]])
   await page.locator(selector).nth(index).screenshot({path:path.join(cache,`ch308-${name}-${width}.png`)});
  results.push({width,errors,...metrics});
 }
 fs.writeFileSync('reports/ch308/browser_proof.json',JSON.stringify(results,null,2)+'\n');
