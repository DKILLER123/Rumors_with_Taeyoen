...
old_audit = json.loads(Path('reports/ch305/full_tree/workspace_audit.json').read_text())
current = {p.relative_to(TREE).as_posix(): sha(p.read_bytes()) for p in TREE.rglob('*') if p.is_file()}
with ZipFile(EPUB) as z:
    infos = z.infolist()
    assert len(infos) == len({i.filename for i in infos}) == 409
    ...
    assert len(items) == len(manifest) == 406
    assert len(spine) == 310 and spine[-1].get('idref') == 'ch306'
    assert all(s.get('idref') in items for s in spine)
    assert items['ch306'].get('href') == 'text/ch306.xhtml'
    ...
    assert len(ncx) == 309 and ncx[-1].get('id') == 'num_309'
    assert ncx[-1].find(N + 'content').get('src') == 'text/ch306.xhtml'
    nav = ET.fromstring(z.read('OEBPS/text/nav.xhtml'))
    assert len(nav.findall('.//' + X + 'li')) == 312
    assert any(a.get('href') == 'ch306.xhtml' and 'When the Gossip Hits Home' in ''.join(a.itertext()) for a in nav.iter(X+'a'))
    chapter = ET.fromstring(z.read('OEBPS/text/ch306.xhtml'))
    assert 'Then he went after Taeyeon.' in ''.join(chapter.itertext())
    assert payload['OEBPS/text/ch306.xhtml'] == pre['chapter_sha256']
    # Verify existing character content, not just five paragraph ownership labels.
    chars = ET.fromstring(z.read('OEBPS/text/characters.xhtml'))
    removed = 0
    for parent in chars.iter():
        for e in list(parent):
            if e.tag == X+'p' and ''.join(e.itertext()).startswith(('In Chapter 306', 'At the end of Chapter 306')):
                parent.remove(e)
                removed += 1
...
for record in old_files.split(b'\0'):
    ...
    raw_upload_count += 1
assert sha(Path('raws/ch306_raw.txt').read_bytes()) == pre['raw_sha256']
...
result = {
    ...
    'archive_entries': 409, 'mimetype_first_stored': True, 'remaining_entries_deflated': True,
    'zip_crc': 'PASS', 'archive_tree_payload_parity': '409/409',
    'unchanged_old_chapters': '305/305', 'unchanged_existing_images_fonts_styles': True,
    ...
    'raw306_sha256': pre['raw_sha256'], 'chapter306_sha256': pre['chapter_sha256'],
    'preserved_ch282_id29_alias': True, 'changed_baseline_payloads': changed, 'added_payloads': added,
    'manifest_items': 406, 'spine_items': 310, 'ncx_navpoints': 309, 'nav_li': 312,
    'decoded_images': 73, 'decoded_woff_faces': 20, 'reference_checks': audit['reference_checks'],
    'structural_errors': [], 'chapter_gate_status': 'PASS; six declarative/imperative regex flags triaged; 19 deliberate recap eight-grams verified',
    'legacy_book_wide_qm_repeat_flags': 'Existing book-wide findings retained; same exit codes and editorial category counts as 305; 306 recap matches explicitly reviewed',
