...
(async()=>{
 ...
 for(const width of [390,800]){
  ...
  await page.goto(pathToFileURL(path.join(root,'work_epub/OEBPS/text/ch306.xhtml')).href);
  ...
  await page.screenshot({path:path.join(cache,`ch306-title-${width}.png`)});
  for(const [name,selector,index] of [['game','.app-screen',0],['dossier','.dossier-block',0],['contract','.contract-block',2],['ending','.acting-block',-1],['whisper','.whisper-block',5]])
   await page.locator(selector).nth(index).screenshot({path:path.join(cache,`ch306-${name}-${width}.png`)});
  results.push({width,errors,...metrics});
 }
 fs.writeFileSync('reports/ch306/browser_proof.json',JSON.stringify(results,null,2)+'\n');
