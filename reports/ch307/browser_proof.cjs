...
(async()=>{
 ...
 for(const width of [390,800]){
  ...
  await page.goto(pathToFileURL(path.join(root,'work_epub/OEBPS/text/ch307.xhtml')).href);
  ...
  await page.screenshot({path:path.join(cache,`ch307-title-${width}.png`)});
  for(const [name,selector,index] of [['phone','.phone-call',0],['dossier','.dossier-block',0],['contract','.contract-block',2],['ending','.system-block',-1],['whisper','.whisper-block',3]])
   await page.locator(selector).nth(index).screenshot({path:path.join(cache,`ch307-${name}-${width}.png`)});
  results.push({width,errors,...metrics});
 }
 fs.writeFileSync('reports/ch307/browser_proof.json',JSON.stringify(results,null,2)+'\n');
