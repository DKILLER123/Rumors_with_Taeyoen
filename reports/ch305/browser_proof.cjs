...
(async()=>{
 ...
 for(const width of [390,800]){
  ...
  await page.goto(pathToFileURL(path.join(root,'work_epub/OEBPS/text/ch305.xhtml')).href);
  ...
  await page.screenshot({path:path.join(cache,`ch305-title-${width}.png`)});
  for(const [name,selector,index] of [['system','.system-block',0],['finance','.finance-block',0],['contract','.contract-block',1],['ending','.acting-block',3],['whisper','.whisper-block',1]])
   await page.locator(selector).nth(index).screenshot({path:path.join(cache,`ch305-${name}-${width}.png`)});
  results.push({width,errors,...metrics});
 }
 fs.writeFileSync('reports/ch305/browser_proof.json',JSON.stringify(results,null,2)+'\n');
