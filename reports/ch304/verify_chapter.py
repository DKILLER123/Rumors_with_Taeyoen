...
for required in ['“I’m willing.”', 'she retrieved', 'My Love from the Star', 'Cheon Song-yi',
                 'Baeksang', 'Blue Dragon', 'next May', 'Especially Taeyeon eonni and Sica eonni',
                 'A name was still missing', 'Not Taeyeon eonni.', 'Pythagorean theorem', 'What a lecher.']:
    assert required.lower() in text.lower(), required
raw = Path('raws/ch304_raw.txt')
raw_hash = hashlib.sha256(raw.read_bytes()).hexdigest()
assert raw_hash == '9fa3cef8c1904343fb7e12bc45de1aa7c0fc4ee201176ef27df9459e816b6fb5'
opf = ET.parse(ROOT / 'OEBPS/content.opf').getroot()
items = opf.findall(O + 'manifest/' + O + 'item')
spine = opf.findall(O + 'spine/' + O + 'itemref')
assert len(items) == 404 and len(spine) == 308
assert spine[-1].get('idref') == 'ch304'
...
assert len(ET.parse(ROOT / 'OEBPS/toc.ncx').findall('.//' + N + 'navPoint')) == 307
assert len(ET.parse(ROOT / 'OEBPS/text/nav.xhtml').findall('.//' + X + 'li')) == 310
...
assert added == ['OEBPS/text/ch304.xhtml']
...
for e in ET.parse(ROOT / 'OEBPS/text/characters.xhtml').iter():
    if e.get('class') == 'char-card':
        additions = [p for p in e.iter(X + 'p') if ''.join(p.itertext()).startswith('In Chapter 304')]
        if additions:
            assert len(additions) == 1
            owners.append(next(''.join(c.itertext()) for c in e.iter() if c.get('class') == 'ci-name'))
assert sorted(owners) == sorted(['Song Ji-ho', 'Im Yoon-a', 'Seohyun (Seo Ju-hyun)'])
census = {c: classes[c] for c in [*headers, 'pullquote']}
assert sum(census.values()) == 26
metrics = {'result':'PASS', 'chapter':304, 'raw_sha256':raw_hash,
           ...
           'added_payloads':added, 'unchanged_old_chapters':'303/303',
           'existing_assets_styles_preserved':True, 'chapter_sha256':now['OEBPS/text/ch304.xhtml']}
